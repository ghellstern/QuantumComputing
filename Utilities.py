...
### Own Utilities
### Own definition of CCCX
def mycccx(qc,q, controls,target,ancillas):
    # need 2 ancillas bits
    qc.ccx(q[controls[0]],q[controls[1]],q[ancillas[0]])
    qc.ccx(q[controls[2]],q[ancillas[0]],q[ancillas[1]])
    qc.cx(q[ancillas[1]],q[target])
    qc.ccx(q[controls[2]],q[ancillas[0]],q[ancillas[1]])
    qc.ccx(q[controls[0]],q[controls[1]],q[ancillas[0]])

    
### Own definition of CCZ
def myccz(qc,q, controls,target,ancillas):
    qc.ccx(q[controls[0]],q[controls[1]],q[ancillas[0]])
    qc.cz(q[ancillas[0]],q[target])
    qc.ccx(q[controls[0]],q[controls[1]],q[ancillas[0]])

### Own definition of CCCZ

def mycccz(qc,q, controls,target,ancillas):
    # need 2 ancillas bits
    qc.ccx(q[controls[0]],q[controls[1]],q[ancillas[0]])
    qc.ccx(q[controls[2]],q[ancillas[0]],q[ancillas[1]])
    qc.cz(q[ancillas[1]],q[target])
    qc.ccx(q[controls[2]],q[ancillas[0]],q[ancillas[1]])
    qc.ccx(q[controls[0]],q[controls[1]],q[ancillas[0]])

### CCCZ Gate without ancillas: see 4-qubit Grover's algorithm
### implemented for the ibmqx5
### architecture
### VERA BLOMKVIST KARLSSON
##  PHILIP STRÖMBERG

def mycccz1(circuit,qr, controls,target):
    import math
    pi=math.pi
